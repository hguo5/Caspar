...
# The training set is separately generated based on a Negative Sampling strategy
...
training_set_name = 'binary_threshold_0.25'
TRAINING_RATIO = 0.9
VECSIZE = 300
BATCH_SIZE = 1
NUM_HIDDEN = 256
...
event_map = {}
for eid1,et1,e1,eid2,et2,e2 in training_raw:
    event_map[eid1] = e1
    event_map[eid2] = e2


training_with_label = []
f = open('data/'+training_set_name+'.csv','r')
for row in f:
    items = row.strip().split(',')
    e1 = event_map[items[0]]
    e2 = event_map[items[1]]
    LABEL = int(items[2])
    training_with_label.append((e1,e2,LABEL))
f.close()
...
save_path = saver.save(sess, 'model/'+training_set_name+'.model')
